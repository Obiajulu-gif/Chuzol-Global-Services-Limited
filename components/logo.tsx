...
export function Logo({ className = "", showText = true, size = "md" }: LogoProps) {
  ...
  return (
    <div className={`flex items-center group ${className}`}>
      <div className={`flex items-center space-x-3 ${!showText ? "space-x-0" : ""}`}>
        ...
        <div className="relative">
          ...
          >
            <Globe className={`${size === "sm" ? "h-4 w-4" : size === "md" ? "h-5 w-5" : "h-6 w-6"} text-white`} />
          </div>
          {/* Circling Arrow Animation */}
          <div className="absolute -top-1 -right-1">
            <div
              className={`${size === "sm" ? "w-4 h-4" : size === "md" ? "w-5 h-5" : "w-6 h-6"} bg-yellow-400 rounded-full flex items-center justify-center animate-spin-slow`}
            >
              <ArrowRight
                className={`${size === "sm" ? "h-2 w-2" : size === "md" ? "h-2.5 w-2.5" : "h-3 w-3"} text-green-800 transform rotate-45`}
              />
            </div>
          </div>
          {/* Pulsing Ring */}
        ...
        {showText && (
          <div className="flex flex-col">
            ...
            <p className={`${size === "sm" ? "text-xs" : "text-xs"} text-gray-600 font-medium`}>Services Limited</p>
          </div>
        )}
      </div>
    </div>
